...
CMD ["/bin/sh", "-c", "/app/.venv/bin/python manage.py migrate --noinput && /app/.venv/bin/python manage.py createsuperuser --noinput || true && /app/.venv/bin/gunicorn backend.wsgi:application --bind 0.0.0.0:8000 --workers 2"]
